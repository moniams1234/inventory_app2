...
def build_summary_table(df: pd.DataFrame) -> pd.DataFrame:
    """
    Buduje pivot podsumowania.

    Wiersze: Magazyn.
    Kolumny: Type of materials, Rodzaj indeksu, Status pozycji (nowa/nabyta).
    Wartości: Kwota rezerwy i Wartość mag.

    Dzięki dodaniu "Status pozycji" arkusz "Podsumowanie" w pliku XLSX
    pokazuje osobno rezerwy oraz wartość magazynową dla pozycji nowych i nabytych.
    """
    ...
    if missing:
        raise ValueError(
            "Brakujące kolumny do zbudowania podsumowania: "
            + ", ".join(missing)
        )

    summary_df = df.copy()
    summary_df["Status pozycji"] = (
        summary_df["Status pozycji"]
        .fillna("do weryfikacji")
        .astype(str)
        .str.strip()
        .replace("", "do weryfikacji")
    )

    agg = summary_df.groupby(
        ["Magazyn", "Type of materials", "Rodzaj indeksu", "Status pozycji"],
        as_index=False,
        dropna=False,
    ).agg(
        **{
    ...
    pivot = agg.pivot_table(
        index="Magazyn",
        columns=["Type of materials", "Rodzaj indeksu", "Status pozycji"],
